...
namespace bpe {
    ...
    string_counter_t pretokenize(const std::string& corpus) {
        ...
        for (auto& tokens : ctre::tokenize<R"('s|'t|'re|'ve|'m|'ll|'d| ?[\p{L}]+| ?[\p{N}]+| ?[^\s\p{L}\p{N}]+|\s+(?!\S)|\s+)">(corpus)) {
    ...
    void train_tokenizer(string_counter_t word_counts, token_t vocab_size) {
        Timer timer;
        timer.start();
        ...
        for (token_t new_token = 256; new_token < vocab_size; new_token++) {
            ...
            std::cout << "Merging: " << ((mx->first.first < 256) ? ((char)mx->first.first) : mx->first.first) << " + " << ((mx->first.second < 256) ? ((char)mx->first.second) : mx->first.second) << " = " << new_token << "\n";
    ...
    void run(const std::string filename) {
        ...
        train_tokenizer(pretoken_counts, 500);
